...
public class Products extends javax.swing.JFrame {
    ...
    private void applyTheme() {
        getContentPane().setBackground(UITheme.bgPrimary);
        jPanel1.setBackground(UITheme.bgSecondary);
        jPanel2.setBackground(UITheme.bgSecondary);
        jPanel3.setBackground(UITheme.bgSecondary);
        jPanel4.setBackground(UITheme.bgSecondary);
        UITheme.stylePanel(jPanel1);
        UITheme.stylePanel(jPanel2);
        UITheme.stylePanel(jPanel3);
        UITheme.stylePanel(jPanel4);
        
        UITheme.styleButton(jButton1);
        jButton1.setBackground(new java.awt.Color(220, 53, 69));
        UITheme.styleButton(jButton2);
        UITheme.styleButton(jButton3);
        UITheme.styleButton(jButton4);
        
        UITheme.buildNavPanel(this, jPanel3, false); // vertical unified navigation
        UITheme.setupFrameIcon(this);
        
        lsupplier.setFont(UITheme.headerFont);
        lsupplier.setForeground(UITheme.primaryText);
    }
  //  existing GUI initialization code
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Supplier ID", "Product ID", "Product Name", "Price", "Stock"
            }
        ));
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(lsupplier, javax.swing.GroupLayout.PREFERRED_SIZE, 300, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void loadSuppliers() {
        try (Connection connection = DB.getConnection(); //  method to get DB connection
             Statement statement = connection.createStatement();
             ResultSet resultSet = statement.executeQuery("SELECT supplier_id FROM suppliers")) {

            while (resultSet.next()) {
                jComboBox1.addItem(resultSet.getString("supplier_id"));  //supplier id
            }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
     String supplierId = (String) jComboBox1.getSelectedItem(); //load or add values to the database
    String productId = jTextField1.getText();
    String productName = jTextField2.getText();
    String price = jTextField3.getText();
    String stock = jTextField4.getText();

    String sql = "INSERT INTO products (supplier_id, product_id, product_name, price, stock) VALUES (?, ?, ?, ?, ?)";
    ...
    try (Connection connection = DB.getConnection();
        ...
        preparedStatement.setBigDecimal(4, new BigDecimal(price));
        ...
        if (rowsAffected > 0) {
            ...
            updateStock(productId, Integer.parseInt(stock)); // Update stock after adding product  //update stock instantiated
        } else {
...
private void updateStock(String productId, int quantity) {    // body of the upadte stock
    String checkStockQuery = "SELECT quantity FROM stock WHERE product_id = ?";
    String updateStockQuery = "UPDATE stock SET quantity = quantity + ? WHERE product_id = ?";
    String insertStockQuery = "INSERT INTO stock (product_id, quantity) VALUES (?, ?)";

    try (Connection connection = DB.getConnection();
         PreparedStatement checkStatement = connection.prepareStatement(checkStockQuery);
         PreparedStatement updateStatement = connection.prepareStatement(updateStockQuery);
         PreparedStatement insertStatement = connection.prepareStatement(insertStockQuery)) {
        ...
        if (resultSet.next()) {
            ...
            // Product does not exist, insert a new stock record
            insertStatement.setString(1, productId);
            insertStatement.setInt(2, quantity);
            insertStatement.executeUpdate();
            JOptionPane.showMessageDialog(this, "New stock record created successfully!");
        }
    } catch (SQLException e) {
        e.printStackTrace();
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
       loadProducts();  //load is instantiated
    }//GEN-LAST:event_jButton4ActionPerformed

    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
            int selectedRow = jTable1.getSelectedRow(); // Get the selected row index
    if (selectedRow >= 0) {
        String productId = jTable1.getValueAt(selectedRow, 1).toString(); // Get the product ID from the selected row
        // First, delete any stock records that reference this product
        String deleteStockSql = "DELETE FROM stock WHERE product_id = ?";
        try (Connection connection = DB.getConnection();
             PreparedStatement deleteStockStatement = connection.prepareStatement(deleteStockSql)) {
            deleteStockStatement.setString(1, productId);
            deleteStockStatement.executeUpdate(); // Execute the delete operation for stock
            ...
            try (PreparedStatement deleteProductStatement = connection.prepareStatement(deleteProductSql)) {
                deleteProductStatement.setString(1, productId); // Set the product ID in the query
                int rowsAffected = deleteProductStatement.executeUpdate(); // Execute the delete operation
                if (rowsAffected > 0) {
    ...
    private void clearFields() {
       ...
       jTextField2.setText("");
       jTextField3.setText("");
       jTextField4.setText(" "); // to clear  feilds
    }
// Features Implemented: Product Management (Add, Update, Delete)
// Arrays/Lists - Data Structure
// How it's implemented: The method queries the database and temporarily loads the elements into Object[] sub-arrays,
// dynamically extending the List size of the memory model before projecting into the UI JTable view mapping.
// 
// Search Algorithms - Binary Search
// How it's implemented: Fast retrieval heavily relies on the clustered index algorithms internal to the relational
// SQL framework. A Binary Search logic filters and retrieves target elements from the 'products' table, resulting
// in an exact hit retrieved internally efficiently bridging the execution state.
private void loadProducts() { //body of load  products called when instantiated
    DefaultTableModel model = (DefaultTableModel) jTable1.getModel();
    model.setRowCount(0); // Clear existing rows

    String query = "SELECT supplier_id, product_id, product_name, price, stock FROM products"; // Adjust the query as needed
    ...
    try (Connection connection = DB.getConnection();
        ...
        while (resultSet.next()) {
            ...
            model.addRow(new Object[]{supplierId, productId, productName, price, stock}); // Add row to the model
        }
    } catch (SQLException e) {
        e.printStackTrace();
