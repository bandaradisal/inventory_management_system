...
public class DemandForecaster {
    ...
    // How it's implemented: A HashMap is used for robust caching of product sales. It maps a unique Integer (product ID)
    // directly to an object holding its specific historic sales data. This avoids iterating through rows sequentially,
    // yielding an extremely efficient O(1) average lookup time when analyzing thousands of products.
    private HashMap<Integer, ProductSalesHistory> salesData;
    
    // Queues / Trees (Binary Heap) - Data Structure
    // How it's implemented: PriorityQueue natively implements a Binary Heap Tree.
    // As recommendations are generated, they are injected into this queue.
    // The internal Tree automatically restructures itself (O(log N)) prioritizing critical "reorder" urgency mathematically,
    // so the most critical items immediately rise to the top of the queue structure for processing.
    ...
    // Arrays/Lists - For storing and managing product inventory history lists
    ...
    // Algorithms Used: Demand Forecasting (Time Series & EOQ)
    // How it's implemented: This mathematical algorithm applies Time Series Analysis by calculating
    // the moving average of daily demand (totalSales / 30). It then executes the Economic Order Quantity (EOQ) formula
    // algorithm, which optimizes costs by balancing "ordering_cost" against "holding_cost" mathematically:
    // EOQ = sqrt((2 * demand * orderCost) / holdCost). This is executed across N elements taking an overall O(N) linear runtime.
